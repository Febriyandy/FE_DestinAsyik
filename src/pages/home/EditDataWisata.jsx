...
const EditDataWisata = () => {
  ...
  const getWisataById = async () => {
    try {
      ...
      if (response.data) {
        ...
        const deskLongText = parseAndReplaceNewlines(data.deskLong || "");
        const hargaText = parseAndReplaceNewlines(data.harga ||"");
        setNama_tempat(data.nama_tempat || "");
        setAlamat(data.alamat || "");
        setKabupaten(data.kabupaten || "");
        setLink_maps(data.link_maps || "");
        setCover(data.cover || null);
        setPreview1(
          data.cover ? `http://localhost:7000/cover/${data.nama_cover}` : ""
        );
        setDeskShort(data.deskShort || "");
        setDeskLong(deskLongText);
        setHarga(hargaText);
        setFoto(data.foto || null);

        const namaFotoArray = JSON.parse(data.nama_foto || "[]");

        setPreviews(
          namaFotoArray.map(
            (namaFoto) => `http://localhost:7000/wisata/${namaFoto}`
          )
        );
      }
    } catch (error) {
  ...
  const editData = async (e) => {
    ...
    try {
      ...
      await axiosJWT.patch(`http://localhost:7000/wisata/${id}`, formData, {
        headers: {
